/**
 * The order aggregate module. Exposes its command/query/view DTOs (in the {@code api.command}/
 * {@code api.query}/{@code api.view} subpackages), {@link
 ...
 * {@code BeerLookup} outbound ports) and the always-open {@code shared} module. A bare module
 * name in {@code allowedDependencies} only covers a target module's default (unnamed) package,
 * not an explicitly declared {@code @NamedInterface} - since customer/product put everything
 * exposed behind a named interface, the dependency must be spelled out as
 * {@code "<module> :: <interface>"}. Its inbound REST adapter also uses the (Modulith-excluded)
 * {@code platform} package's {@code rest}/{@code security} types ({@code ErrorDetails}/
 * {@code Authorized}).
 */
