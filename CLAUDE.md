...
- `domain/<aggregate>/` (e.g. `customer`, `order`) — the aggregate root service (`Customers`, `Orders`), `data/` (JPA `@Entity` classes, always suffixed `Data`, never `Entity` — entities are "stupid data bags"), `repository/` (plain repository *ports*, `@Repository`-annotated but framework-agnostic — no `extends JpaRepository`, only the methods the domain actually calls), `value/` (value objects/enums). The entities stay JPA-annotated by deliberate choice: this PoC does not maintain a separate persistence model, so the JPA entity doubles as the domain model. `DomainArchitectureTest` has an explicit ArchUnit rule that the domain must not depend on `org.springframework.data..` — the repository ports are the one place that boundary is enforced.
...
- `adapter/out/persistence/` — the driven/outbound side: for each aggregate, a package-visible Spring Data interface (`CustomerJpaRepository extends JpaRepository<...>`) plus a `*PersistenceAdapter` (`@Repository`, implements the domain's repository port, delegates to the Spring Data interface). This is where the actual Spring Data dependency lives now, kept out of `domain`.
